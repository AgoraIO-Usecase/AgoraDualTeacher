...
struct SAudioFrame : public SMediaFrame {
  ...
  SAudioFrame() : SMediaFrame(), codec_(0), ts_(0), vad_(0), internalFlags_(0) {}
};
...
struct broadcast_packet_t : public rtc_packet_t {
  bool quit;
  bool rtcp;
  broadcast_packet_t() : quit(false), rtcp(false) {}
};
...
struct audio_packet_t : public rtc_packet_t {
  ...
  audio_packet_t() : ts(0), vad(0), codec(0), last_error(0), reqMs(0) {}
};
...
public:
    ...
    enum VideoFeedbackPacketType {
      ...
      VIDEO_FEEDBACK_BANDWIDTH_ESTIMATION = 5,
      VIDEO_FEEDBACK_REQUEST_MULTISTREAM = 6,
      VIDEO_FEEDBACK_AUT = 7,
    };
    struct PacketInfo
    {
      const void* packet;
    ...
    // new interface for feedback message
