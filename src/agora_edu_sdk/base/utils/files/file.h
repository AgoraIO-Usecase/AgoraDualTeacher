...
class File {
 public:
  ...
  // too, and the ParamTraits<agora::utils::File::Info> implementation in
  ...
  bool SetTimes(int64_t last_access_time, int64_t last_modified_time);
  ...
  // closest agora::utils::File::Error equivalent via OSErrorToFileError(). The returned
  // value is only trustworthy immediately after another agora::utils::File method
  // fails. agora::utils::File never resets the global error to zero.
