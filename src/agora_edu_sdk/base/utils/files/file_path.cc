...
const FilePath::CharType kStringTerminator = ('\0');
...
StringType::size_type ExtensionSeparatorPosition(const StringType& path) {
  ...
  for (size_t i = 0; i < arraysize(kCommonDoubleExtensions); ++i) {
    ...
    if (LowerCaseEqualsASCII(extension, kCommonDoubleExtensions[i])) return penultimate_dot;
  }
...
bool FilePath::IsFileExist(const std::string& f) {
  if (f.empty()) {
    return false;
  }

  std::ifstream infile(f.c_str(), std::ios_base::in);
  if (!infile.is_open()) {
    return false;
  }
  infile.close();
...
FilePath FilePath::CurrentModule() {
#if defined(OS_WIN)
  std::wstring module;
  std::unique_ptr<wchar_t[]> path = std::make_unique<wchar_t[]>(260);
  HMODULE hModule = GetModuleHandle(NULL);
  GetModuleFileName(hModule, path.get(), MAX_PATH);
  module = path.get();
#else
  std::string module;
  Dl_info info;
  // Warning: this only works when build with -fPIC is target is a shared object
  // otherwise suprise happens (see `man dladdr` for more information)
  int r = dladdr(reinterpret_cast<void*>(&FilePath::CurrentModule), &info);
  if (r && info.dli_fname) {
    module = info.dli_fname;
  }

  if (!module.empty()) {
    char* resolved = realpath(module.c_str(), nullptr);
    if (resolved) {
      module = resolved;
      free(resolved);
    }
...
#if defined(OS_WIN)
FilePath::FilePath(WStringPieceType path) {
  int size = WideCharToMultiByte(CP_UTF8, 0, path.data(), -1, nullptr, 0, nullptr, nullptr);
  if (!size) return;
  std::unique_ptr<char[]> converted = std::make_unique<char[]>(260);
  size = WideCharToMultiByte(CP_UTF8, 0, path.data(), path.length() + 1, converted.get(), 260,
                             nullptr, nullptr);
  if (size) {
    StringPieceType path(converted.get());
    path.CopyToString(&path_);
    StringType::size_type nul_pos = path_.find(kStringTerminator);
    if (nul_pos != StringType::npos) path_.erase(nul_pos, StringType::npos);
  }
...
bool FilePath::IsParent(const FilePath& child) const { return AppendRelativePath(child, nullptr); }
...
bool FilePath::AppendRelativePath(const FilePath& child, FilePath* path) const {
  ...
  if ((FindDriveLetter(*parent_comp) != StringType::npos) &&
    ...
    if (!StartsWith(*parent_comp, *child_comp, CompareCase::INSENSITIVE_ASCII)) return false;
    ++parent_comp;
    ++child_comp;
  }
#endif  // defined(FILE_PATH_USES_DRIVE_LETTERS)
...
std::string FilePath::AsUTF8Unsafe() const { return value(); }

// static
FilePath FilePath::FromUTF8Unsafe(StringPiece utf8) { return FilePath(utf8); }

#if defined(OS_WIN)
// Windows specific implementation of file string comparisons.

int FilePath::CompareIgnoreCase(StringPieceType string1, StringPieceType string2) {
  int comparison = stricmp(string1.data(), string2.data());
...
bool FilePath::IsContentUri() const {
  return StartsWith(path_, "content://", CompareCase::INSENSITIVE_ASCII);
}
#endif
