...
class FilePath {
 public:
  static FilePath CurrentModule();

 public:
  typedef std::string StringType;
  typedef std::wstring WStringType;
  typedef BasicStringPiece<StringType> StringPieceType;
  typedef BasicStringPiece<WStringType> WStringPieceType;
  typedef StringType::value_type CharType;
  typedef WStringType::value_type WCharType;
  ...
  static bool IsFileExist(const std::string& f);

  FilePath();
  FilePath(const FilePath& that);
  explicit FilePath(StringPieceType path);
#if defined(OS_WIN)
  explicit FilePath(WStringPieceType path);
#endif
  ~FilePath();
...
template <>
struct hash<agora::utils::FilePath> {
  typedef agora::utils::FilePath argument_type;
  typedef std::size_t result_type;
  result_type operator()(argument_type const& f) const {
    return hash<agora::utils::FilePath::StringType>()(f.value());
  }
};
