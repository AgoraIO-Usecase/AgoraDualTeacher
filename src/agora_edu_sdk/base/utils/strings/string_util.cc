      ...
      while (in_specification) {
        ...
        if (*position == 'l') {
          ...
          modifier_l = true;
                   ...
                   *position == 'C' || *position == 'F' || *position == 'D' || *position == 'O' ||
                   *position == 'U') {
          // Not portable.
          return false;
        }
...
StringType ToLowerASCIIImpl(BasicStringPiece<StringType> str) {
  ...
  for (size_t i = 0; i < str.size(); i++) ret.push_back(ToLowerASCII(str[i]));
  return ret;
}
...
StringType ToUpperASCIIImpl(BasicStringPiece<StringType> str) {
  StringType ret;
  ret.reserve(str.size());
  for (size_t i = 0; i < str.size(); i++) ret.push_back(ToUpperASCII(str[i]));
  return ret;
}

}  // namespace

std::string ToLowerASCII(StringPiece str) { return ToLowerASCIIImpl<std::string>(str); }

std::string ToUpperASCII(StringPiece str) { return ToUpperASCIIImpl<std::string>(str); }

template <class StringType>
int CompareCaseInsensitiveASCIIT(BasicStringPiece<StringType> a, BasicStringPiece<StringType> b) {
...
template <class StringType>
bool ReplaceCharsT(const StringType& input, BasicStringPiece<StringType> find_any_of_these,
...
TrimPositions TrimStringT(const Str& input, BasicStringPiece<Str> trim_chars,
  ...
  const size_t first_good_char =
      (positions & TRIM_LEADING) ? input_piece.find_first_not_of(trim_chars) : 0;
  const size_t last_good_char =
      (positions & TRIM_TRAILING) ? input_piece.find_last_not_of(trim_chars) : last_char;
...
bool IsStringASCII(StringPiece str) { return DoIsStringASCII(str.data(), str.length()); }
...
static inline bool DoLowerCaseEqualsASCII(BasicStringPiece<Str> str, StringPiece lowercase_ascii) {
  ...
  for (size_t i = 0; i < str.size(); i++) {
    if (ToLowerASCII(str[i]) != lowercase_ascii[i]) return false;
  }
  return true;
}
...
template <typename Str>
bool StartsWithT(BasicStringPiece<Str> str, BasicStringPiece<Str> search_for,
...
template <typename Str>
bool EndsWithT(BasicStringPiece<Str> str, BasicStringPiece<Str> search_for,
               CompareCase case_sensitivity) {
  if (search_for.size() > str.size()) return false;

  BasicStringPiece<Str> source = str.substr(str.size() - search_for.size(), search_for.size());
...
bool DoReplaceMatchesAfterOffset(StringType* str, size_t initial_offset, Matcher matcher,
  ...
  if (!find_length) return false;

  // If the find string doesn't appear, there's nothing to do.
  size_t first_match = matcher.Find(*str, initial_offset);
  if (first_match == StringType::npos) return false;
  ...
  if (replace_length > find_length) {
    ...
    // |shift_dst|.
    if (shift_dst > str_length) str->resize(shift_dst);
...
bool ReplaceCharsT(const StringType& input, BasicStringPiece<StringType> find_any_of_these,
  ...
  *output = input;
  ...
  return DoReplaceMatchesAfterOffset(output, 0, CharacterMatcher<StringType>{find_any_of_these},
...
void ReplaceFirstSubstringAfterOffset(std::string* str, size_t start_offset, StringPiece find_this,
  ...
  DoReplaceMatchesAfterOffset(str, start_offset, SubstringMatcher<std::string>{find_this},
...
static string_type JoinStringT(const list_type& parts, BasicStringPiece<string_type> sep) {
  if (parts.size() == 0) return string_type();
  ...
  AppendToString(&result, string_type(*iter));
  ++iter;
...
OutStringType DoReplaceStringPlaceholders(const FormatStringType& format_string,
  ...
  size_t sub_length = 0;
  for (const auto& cur : subst) sub_length += cur.length();
    ...
    if ('$' == *i) {
      if (i + 1 != format_string.end()) {
        ...
        if ('$' == *i) {
          ...
          if (index < substitutions) formatted.append(subst.at(index));
        }
      }
    } else {
  ...
  if (offsets) {
    for (const auto& cur : r_offsets) offsets->push_back(cur.offset);
  }
  return formatted;
}
