...
class ByteReader<T, B, false> {
 ...
 private:
  static T InternalReadBigEndian(const uint8_t* data) {
    T val(0);
    for (unsigned int i = 0; i < B; ++i) val |= static_cast<T>(data[i]) << ((B - 1 - i) * 8);
    return val;
  }

  static T InternalReadLittleEndian(const uint8_t* data) {
    T val(0);
    for (unsigned int i = 0; i < B; ++i) val |= static_cast<T>(data[i]) << (i * 8);
    return val;
  }
};
...
class ByteWriter<T, B, true> {
 public:
  ...
  static void WriteLittleEndian(uint8_t* data, T val) {
    ByteWriter<U, B, false>::WriteLittleEndian(data, ReinterpretAsUnsigned(val));
  }
...
class ByteWriter<T, 4, false> {
 public:
  static void WriteBigEndian(uint8_t* data, T val) {
    static_assert(sizeof(T) >= 4, kSizeErrorMsg);
    data[0] = static_cast<uint8_t>(val >> 24);
    data[1] = static_cast<uint8_t>(val >> 16);
    data[2] = static_cast<uint8_t>(val >> 8);
    data[3] = static_cast<uint8_t>(val);
  }
...
class ByteReader<T, 8, false> {
 public:
  ...
  static T ReadLittleEndian(const uint8_t* data) {
    static_assert(sizeof(T) >= 8, kSizeErrorMsg);
    return Get(data, 0) | (Get(data, 1) << 8) | (Get(data, 2) << 16) | (Get(data, 3) << 24) |
           (Get(data, 4) << 32) | (Get(data, 5) << 40) | (Get(data, 6) << 48) |
           (Get(data, 7) << 56);
  }
...
class ByteWriter<T, 8, false> {
 public:
  static void WriteBigEndian(uint8_t* data, T val) {
    static_assert(sizeof(T) >= 8, kSizeErrorMsg);
    data[0] = static_cast<uint8_t>(val >> 56);
    data[1] = static_cast<uint8_t>(val >> 48);
    data[2] = static_cast<uint8_t>(val >> 40);
    data[3] = static_cast<uint8_t>(val >> 32);
    data[4] = static_cast<uint8_t>(val >> 24);
    data[5] = static_cast<uint8_t>(val >> 16);
    data[6] = static_cast<uint8_t>(val >> 8);
    data[7] = static_cast<uint8_t>(val);
  }
  ...
  static void WriteLittleEndian(uint8_t* data, T val) {
    ...
    data[2] = static_cast<uint8_t>(val >> 16);
    data[3] = static_cast<uint8_t>(val >> 24);
    data[4] = static_cast<uint8_t>(val >> 32);
    data[5] = static_cast<uint8_t>(val >> 40);
    data[6] = static_cast<uint8_t>(val >> 48);
    data[7] = static_cast<uint8_t>(val >> 56);
  }
};
